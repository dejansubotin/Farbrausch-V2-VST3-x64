...
namespace
{
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////
	{
		if (!smplrem || !smpldelta || !td2 || nexttime <= time)
		{
			if (smpldelta)
				*smpldelta = 0;
			if (smplrem)
		...
		const sU64 numerator =
			static_cast<sU64>(nexttime - time) * static_cast<sU64>(usecs) +
			static_cast<sU64>(*smplrem);
		*smpldelta = static_cast<sU32>(numerator / static_cast<sU64>(td2));
		*smplrem = static_cast<sU32>(numerator % static_cast<sU64>(td2));
	}
}
...
//////////////////////////////////
{
	...
	if (m_tpc)
	{
		destsmpl = static_cast<sU32>(
			(static_cast<sU64>(a_time) * static_cast<sU64>(m_samplerate)) /
			static_cast<sU64>(m_tpc));
	}
...
//////////////////////////////////////
{
	...
	if (a_fadetime)
	{
		...
		if (m_tpc)
		{
			ftsmpls = static_cast<sU32>(
				(static_cast<sU64>(a_fadetime) * static_cast<sU64>(m_samplerate)) /
				static_cast<sU64>(m_tpc));
		}
		if (ftsmpls)
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (!a_add)
    {
			std::memset(a_buffer, 0, sizeof(sF32) * a_len * 2);
    }
	}
	else
	{
...
sU32 V2MPlayer::GetLengthMs() const
{
	if (!m_base.valid || !m_tpc)
		return 0;

	return static_cast<sU32>(
		(static_cast<sU64>(m_base.maxtime) * 1000ULL) /
		static_cast<sU64>(m_tpc));
}

sU32 V2MPlayer::GetPlaybackTimeMs() const
{
	if (!m_base.valid || !m_samplerate)
		return 0;

	const sS64 totalSamples = static_cast<sS64>(m_timeoffset) + static_cast<sS64>(m_state.cursmpl);
	if (totalSamples <= 0)
		return 0;

	const sU64 positionMs =
		(static_cast<sU64>(totalSamples) * static_cast<sU64>(m_tpc)) /
		static_cast<sU64>(m_samplerate);
	const sU32 lengthMs = GetLengthMs();
	return (lengthMs && positionMs > lengthMs) ? lengthMs : static_cast<sU32>(positionMs);
}
